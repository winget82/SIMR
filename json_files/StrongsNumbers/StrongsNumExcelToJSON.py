...
strkjvot = load_workbook('P:/python_work/nates/SIMR/reference_files/StrongsNumbers/NT/READY/STRONGSNMBRS_KJV_1CORINTHIANS.xlsx')#UPDATE THIS FOR EACH BOOK
sheet = strkjvot.active

#READ EACH ROW OF CELL(S) IN FROM EXCEL SHEET AND APPEND TO LIST ADDING SPACE AT END
for row in sheet.iter_rows(min_row=1, min_col=1, max_row=1440, max_col=1):#UPDATE THIS FOR EACH BOOK
	for cell in row:
...
book = '1 Corinthians'#UPDATE THIS FOR EACH BOOK
...
#WRITE TO JSON FILE
filename = '1Corinthians_json.json'
